...
const INAPlatform = {
  ...
  async createProject({ name, projectType, programId, country, description, beneficiaryCount }) {
    const session = await this.getSession();
    if (!session) throw new Error('Not signed in.');
    const { data, error } = await supabaseClient
      .from('projects')
  ...
  async updateProject(id, { name, projectType, programId, country, description, beneficiaryCount }) {
    const { data, error } = await supabaseClient
      .from('projects')
      .update({
        name,
