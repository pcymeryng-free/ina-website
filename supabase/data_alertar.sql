-- ============================================================================
-- Alta de proyecto: Sistema de Alerta Temprana - AlertAR
-- Fuente: PLIEG-2026-63921688-APN-DNIERYSTIYC%ENACOM.pdf
--         (Pliego de Especificaciones Técnicas — Licitación "llave en mano")
--
-- QUE ES ESTE DOCUMENTO
-- A diferencia de los proyectos cargados anteriormente (propuestas de
-- terceros dirigidas a ENACOM), este es el Pliego de Especificaciones
-- Técnicas que el propio ENACOM emite para LICITAR la contratación llave
-- en mano del "SISTEMA DE ALERTA TEMPRANA - AlertAR": un sistema nacional
-- de alertas públicas basado en Cell Broadcast Service (CBS), que se
-- entregará en propiedad al MINISTERIO DE SEGURIDAD NACIONAL para uso del
-- SINAGIR (Sistema Nacional para la Gestión Integral del Riesgo). El
-- proyecto específico que le da origen es IF-2025-117577290-APN-
-- DNPYD#ENACOM, aprobado por Resolución RESOL-2025-1387-APN-ENACOM#JGM del
-- 18/11/2025.
--
-- Por eso generating_entity_name/type queda en ENACOM (organismo
-- contratante/regulador emisor del pliego) en vez de en un proponente
-- externo, y el project_type 'early_warning_system' encaja exactamente
-- con el tipo agregado en migration_v25 para este tipo de sistemas.
--
-- CÓMO USAR ESTE ARCHIVO
-- 1. Supabase → SQL Editor → New query.
-- 2. EDITAR el email de la cuenta que debe figurar como dueña del proyecto
--    (se dejó 'pcymeryng@gmail.com', el mismo usado en los proyectos
--    anteriores de esta sesión — cambialo si corresponde).
-- 3. Ejecutá el script completo. Al final corre un SELECT de verificación.
-- 4. El PDF no se puede adjuntar por SQL — instrucciones al pie del
--    archivo.
-- ============================================================================
...
$desc$TEMPLATE DE PROYECTO DE SISTEMA DE ALERTA TEMPRANA
...
Fuente: "Pliego de Especificaciones Tecnicas" - Licitacion PLIEG-2026-63921688-APN-DNIERYSTIYC#ENACOM (documento adjunto en la seccion de documentos tecnicos del proyecto).$desc$,
  ...
  -- beneficiary_count: no aplica un numero concreto - el pliego no
  -- especifica una cifra de poblacion beneficiaria; el alcance es
  -- nacional, limitado por la cobertura de red movil de los tres
  -- PRESTADORES.
  null,

  'ENACOM - Ente Nacional de Comunicaciones',

  -- ENACOM es el organismo contratante/regulador que emite este pliego de
  -- licitacion (a diferencia de los proyectos anteriores, presentados A
  -- ENACOM por terceros) - 'regulator' es el valor mas preciso de la
  -- taxonomia existente.
  'regulator',

  -- Respuestas del Template de Sistema de Alerta Temprana
  -- (assets/platform.js -> EARLY_WARNING_TEMPLATE) respaldadas por el
  -- pliego. No se completaron destinatario_presentacion (no aplica: este
  -- documento no es una propuesta presentada A ENACOM, sino el pliego que
  -- ENACOM mismo emite), monto_total_solicitado (el pliego no publica un
  -- presupuesto oficial; el monto surge de la oferta economica de cada
  -- oferente), tiempo_objetivo_difusion (no se especifica en segundos/
  -- minutos) ni campana_concientizacion_publica (no descripta en las
  -- secciones relevadas).
$$
{
...
-- Verificación: confirma que el proyecto se creó y muestra su id.
...
-- 2. Abrí el proyecto "Sistema de Alerta Temprana - AlertAR".
-- 3. En la sección de Documentos, subí el archivo
--    "0010 - PLIEG-2026-63921688-APN-DNIERYSTIYC%ENACOM.pdf" en la
--    categoría "Técnica" (technical) — es un Pliego de Especificaciones
--    Técnicas.
-- 4. Opcional pero recomendado: desde la sección de Templates del
--    proyecto, abrí "Usar template" → Template de Sistema de Alerta
--    Temprana. Vas a ver los campos de arriba ya precargados (gracias a
--    shared_field_answers) — completá ahí lo que el pliego no define
--    (monto total de la oferta económica una vez adjudicada, campaña de
--    concientización pública, tiempo objetivo de difusión si se define
--    más adelante).
-- ============================================================================
